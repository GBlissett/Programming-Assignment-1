    ...
    if (int_selection >= 65 && int_selection <= 67) //ie if int_selection (ie ASCII code) is inisde the range for char A - C (ie ASCII 65 - 67)
    {
        switch (selection)
       {
        case 'A':
            ...
            //menu that allows user to specify what type of cipher they would like to use to encrypt message 
            printf("Select type of cipher: \n");
            printf("    a: Rotation cipher\n");
            printf("    b: Substitution cipher\n\n");
            printf("Selection: \n"); 
            getchar(); //to prevent carrying of char from previous menu
            scanf("%c", &cipher);
            Encryp_alg_mess_txt_key(cipher); 
            /*
             * Carries value of char cipher to function definition that allows user to select what algorithm thay want to use to encrypt provided 
             * message
             */
            
        case 'B':       
            printf("Case B Selected\n\n");
            //menu that allows user to specify what type of cipher they would like to use to decrypt message     
            printf("Select type of cipher: \n");
            printf("    a: Rotation cipher\n");
            printf("    b: Substitution cipher\n\n");
            printf("Selection: "); 
            ...
            /*
             * Carries value of char cipher to function definition that allows user to select what algorithm thay want to use to decrypt provided 
             * message
             */
            break;
        ...
        case 'C':
            ...
            //menu that allows user to specify what type of cipher they would like to use to decryot message 
            printf("Select type of cipher: \n");
            printf("    a: Rotation cipher\n");
            printf("    b: Substitution cipher\n\n");
            printf("Selection: "); 
            getchar(); //prevents carrying of char from previous menu
            scanf("%c", &cipher);
            ...
            if (cipher == 'a')
            {
                ...
                /*
                 * Carries value of char cipher to function definition that decrypts provided message by perfroming frequency analysis to deduce 
                 * rotation key
                 */
            }
            else if (cipher == 'b')
...
/*
* The following function allows user to select what algorithm thay want to use to encrypt provided message by taking char input by user (ie either 
* 'a' or 'b')
*/
void Encryp_alg_mess_txt_key(char cipher)
{
    ...
    if (int_cipher >= 97 && int_cipher <= 99)
    {
        switch (cipher)
       {
        case 'a':
            ...
            /*
             * Executes function that generates two arrays; array of origional letters and array of corresponding encrypted letters according to the 
             * rotation shift (ie k) of alphabet input by program user
             */
            break;
        ...
        case 'b':     
            ...
            /*
             * Executes function that generates two arrays; array of origional letters and array of corresponding encrypted letters according letter 
             * assignment input by program user
             */
            break;
        }
    }
...
/*
* The following function allows user to select what algorithm thay want to use to decrypt provided message by taking char input by user (ie either 
* 'a' or 'b')
*/
void Decryp_alg_ciph_txt_key(char cipher)
{
 ...
 * (ie k) of alphabet input by program user. These arrays are then carried to function which performs encryption of message.
*/
void Rotation_cipher_Encrypt(void)
{
   ...
   for (i = 0; i < 26; i++)
   {
    ...
    Encryption(orig_letters, letters); //executes function that takes newly generated arrays letters[i] and orig_letters[i] and encrypts the message from origional.txt
}

/*
 * The following function generates two arrays; array of origional letters and array of corresponding encrypted letters according to the rotation shift (ie k) of 
 * alphabet input by program user. These arrays are then carried to function which performs decryption of cipher text.
 */
void Rotation_cipher_Decrypt(void)
{
 ...
 * user. Function allows user to manually input what each encrypted letter a specific letter will appear as in the final encrypted message.
*/
void Substitution_cipher_Encrypt(void)
{
...
/*
 * The following function generates two arrays; array of origional letters and array of corresponding encrypted letters according letter assignment input by 
 * program user. Function allows user to manually input what each encrypted letter a specific letter will appear as in the final encrypted message.
 */
void Substitution_cipher_Decrypt(void)
{
   int i, letters[alph], orig_letters[alph];
...
/*
 * The following functin that decrypts given cipher text previously encryted using rotation cipher. It does this by determining how many times eash letter 
 * of the alphabbet is used within the encrypted message. According to frequency analysis, the letter E typically has a frequency of 12.02%, at least 3% 
 * higher than any other letter. As such, this frquency is used to deduce the rotation key by caculating the difference between the ASCII value of the most 
 * frequent letter present in the encrypted message and the ASCII value for the letter E (ie 69). This shift value is then used to generates two arrays; 
 * array of origional letters and array of corresponding encrypted letters according to the rotation shift (ie k) of alphabet input by program user. These 
 * arrays are then carried to function which performs encryption of message. 
 */
void Decryp_ciph_txt_no_key_Rotation(void)
{
  ...
  input = fopen("encrypted.txt", "r");
        ...
        while(!feof(input))
	   {
        ...
        k = 26 - (freq_num - 69); 
        /*
         * Rotation key (i k) is calculated by finding the difference between the ASCII value of the most frequent letter present in the encrypted message 
...
/*
 * The following function performs decryption of given cipher text previously encryted using substitution cipher using frequancy analysis of letters, words and letter 
 * combinations. Such frequency analysis allows for the deduction of specific letters.
 */
void Decryp_ciph_txt_no_key_Substitution(void)
{
  ...
  input = fopen("encrypted.txt", "r");
	...
	/*
     * The following for loop determines the letter that occurs before letters[4] (ie letter 'E'), letters[13] (ie letter 'N'), letters[19] (ie letter 'T') and 
