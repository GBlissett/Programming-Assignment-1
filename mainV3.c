 ...
 *input by program user
 */                                 
void Encryption(int orig_letters[alph], int letters[alph]);
/*
...
*/
void Decryption(int orig_letters[alph], int letters[alph]); 
/*
...
int main ()
{
    ...
    printf("Please enter message ALL IN CAPITAL LETERS into file origional.txt or encryoted.txt depending if \nyou wish for the message to be encrypted or decrypted\n\n");
    /*
    ...
    // Menu that allows user to select if they want to encrypt or decrypt a message (depending on the information they have)
    ...
    printf("    C: Decryption given cipher text and algorthm used to encrypt message without the key\n\n");  
    printf("Selection: ");
    scanf("%c", &selection);
    ...
    if (int_selection >= 65 && int_selection <= 67) //IF statement ensures user input value is within the range for char A - C (ie ASCII 65 - 67)
    {
        switch (selection)
       {
        case 'A':
    ...
    else if (int_selection < 65 || int_selection > 67) //ELSE IF statement ensures user input value is within the range for char A - C (ie ASCII 65 - 67)
    {
        printf("Please make valid selection"); 
        return 0; //stops program if user makes an invalid selection (ie does not specify 'A','B' or 'C')
    }
...
void Rotation_cipher_Encrypt(void)
{
   ...
   for (i = 0; i < 26; i++)
   {
    ...
    Encryption(letters, orig_letters);
}
