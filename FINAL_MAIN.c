...
int main ()
{
    ...
    /*
                ...
                {
                    printf("Make possible selection\n"); //stops program if user makes an invalid selection (ie does not specify a or b)
    ...
    {
        printf("Please make valid selection\n"); //stops program if user makes an invalid selection (ie does not specify A, B or C)
        return 0;
    }
 ...
 * char 'cipher' entered by user (ie either 'a' or 'b') into Alorithm Specifier Menu (ie input is char 'cipher'). The return value is the extecution of a 
 * specific funtion depending on values user enters (ie selection of specific options within menu - See above menu's).
 * NB this function does not carry any function values to other functions and stops program if user enters invalid value
 */
void Encryp_alg_mess_txt_key(char cipher)
{
    int int_cipher;
    ...
    if (int_cipher >= 97 && int_cipher <= 99)
    {
        switch (cipher)
        {
            case 'a':
                printf("Case a selected\n\n");  
                return Rotation_cipher_Encrypt(); 
                /*
            ...
            case 'b':     
                printf("Case b Selected\n\n");
                return Substitution_cipher_Encrypt(); 
                /*
    ...
    {
        printf("Please make possible selection\n"); //stops program if user makes an invalid selection (ie does not input 'a' or 'b')
...
void Decryp_alg_ciph_txt_key(char cipher)
{
    ...
    if (int_cipher >= 97 && int_cipher <= 99)
    {
        switch (cipher)
        {
            case 'a':
                printf("Case a selected\n\n");  
                return Rotation_cipher_Decrypt(); 
                            /*
            ...
            case 'b':     
                printf("Case b Selected\n\n");
                return Substitution_cipher_Decrypt(); 
                /*
 ...
 * and retuns encryption of message. Thus, the return value is the execution of this function.
 * NB this function has no inputs. Limitations of this function include that only a number between 1-26 can be entered by program user or fucntion does not work
 */
void Rotation_cipher_Encrypt(void)
{
   ...
   for (i = 0; i < 26; i++)
   {
    ...
    return Encryption(letters, orig_letters); //executes function that takes newly generated arrays letters[i] and orig_letters[i] and encrypts the message from origional.txt
}
 ...
 * and retuns decryption of message. Thus, the return value is the execution of this function.
 * NB this function has no inputs. Limitations of this function include that only a number between 1-26 can be entered by program user or fucntion does not work and if 
 * program user enters incorrect rotation key, incorrect decryption will be made.
 */
void Rotation_cipher_Decrypt(void)
{
   ...
   for (i = 0; i < 26; i++)
   {
    ...
    return Decryption(letters, orig_letters); //executes function that takes newly generated arrays letters[i] and orig_letters[i] and decrypts the message from encrypted.txt
}
 ...
 * carried to function 'Encryption(letters[alph], orig_letters[alph])' which performs and retuns encryption of message. Thus, the return value is the execution of this 
 * function. Thus, the return value is the execution of this function.
 ...
 * The following function generates two arrays; array of origional letters and array of corresponding encrypted letters according letter assignment input by program
 * user. Function allows user to manually input what each encrypted letter a specific letter will appear as in the final encrypted message. These arrays are then 
 * carried to function 'Decryption(letters[alph], orig_letters[alph])' which performs and retuns decryption of message. Thus, the return value is the execution of this 
 * function.
 * NB this function has no inputs
 */
void Substitution_cipher_Decrypt(void)
 ...
 * However, if the most common letter within the meesage (ie origninal message) is not 'E', this code will not work. As such a WHILE loop is also included in this function which
 * allows for the decryption of the message (ie cipher text) by getting the program user to manually specify is a certain decryption using a specfic key (ie k) successfully decrypted 
 * the message (ie by inputting 1 for YES or 0 for NO). If the user specifys that the message was not decrypted correctly (ie by entering 0),the value of k is increased on each
 * repetition of the WHILE loop. If user specfiys that decryption was successful (ie by enterieng 1), the loop (and in turn the program) stops. This shift value (ie key (k)) is then 
 * used to generates two arrays; array of origional letters and array of corresponding encrypted letters according to the rotation shift (ie key (k)) of alphabet input by program user.
 * These arrays are then carried to function'Decryption(letters[alph], orig_letters[alph])' which performs and retuns decryption of message. 
 * NB This function has no inputs and the number of times a user can enter 0 to specify that the decryption was incorrect is limited as sue to the nature of Eclipse Che,the prgram is
 * killed after a certain value is entered/computated/repeated 16 times, therefore limiting the programs ability of the WHILE loop to decrypt some messages if k < 16.
 */
void Decryp_ciph_txt_no_key_Rotation(void)
{
   int k, i, letters[alph], under, orig_letters[alph], size = 500, count, freq, freq_number, key, j, correct;
  ...
  while(!feof(input))
  {
      ...
      for(i = 0; i < size - 1; i++)
      {
          ...
          if ((key >= 65) && (key <= 90) && (key != freq_number))
          {
              for(j = i + 1; j < size; j++)
              {
      ...
      printf("Num = %d and times = %d\n\n", freq_number, count); //prints how many times each letter is used within message 
      break;
  }
  ...
  for (i = 0; i < 26; i++)
  {
   ...
   Decryption(orig_letters, letters); //executes function that takes newly generated arrays letters[i] and orig_letters[i] and decrypts the message from encrypted.txt
    ...
    printf("Is decryption correct? (ie 1 = YES, 0 = NO)\n");
    scanf("%d", &correct);
    
    /*
 * The following WHILE loop allows for the decryption of the message (ie cipher text) if the most frequent letter withinthe message is not 'E'. It does this through
 * getting the program user to manually specify is a certain decryption using a specfic key (ie k) successfully decrypted the message. If the message is not 
 * decrypted correctly,the value of k is increased on each repetition of the loop. If decryption is successful,the loop (and in turn program) stops.
 */
while (correct == 0)
{
    for (k = 1; k <= 26; k++)
 ...
 *  - using frequancy analysis of letters, words and letter combinations (See FOR loops bellow for specifics)
 ...
 * appear as in the encrypted message  of alphabet input by program user. These arrays are then carried to function 'Decryption(letters[alph], orig_letters[alph])'
 * which performs and retuns decryption of message. Thus, the return value is the execution of this function.
 * NB This function has no inputs and is not able to fully decrypt message. This inability to preform full decryption is because code is incomplete and the ability of 
 * this function to decrypt the message (ie cipher text) is heavily dependent on the words used within the message, the frequency of certain letters and the order in which 
 * words appear in the text (since FOR loops test for letters/letter combinations/words beginning at the start of message then continue till the end until 
 * letters/letter combinations/words is found). If letters/letter combinations/words is not found, the letter cannot be deduced, limiting the ability of the following FOR
 * loops to correctly deduce other letters.
 */
void Decryp_ciph_txt_no_key_Substitution(void)
{
  ...
  while(!feof(input))
  {
    ...
    /*
     * The following FOR loop determines the letter that occurs before and after a space (ie ' ') and does not equalletters[8] (ie letter 'I') then assigns it to
     *  letters[0] (ie letter 'A'). In other words, loop finds word ' A ' within the text     
     */
	for (I = 0; I < size - 1; I++)
	{
	...
	return Decryption(letters, orig_letters);
 ...
 * encrypts the message from origional.txt according to the two arrays, printing it to console and to encrypted.txt. The return is the exiting/ending of the 
 * function/program.
 * NB an incorrect encryotion will be made if arrays have letters stored in incorrect order (ie functions that output arrays letters and orig_letters to this function does
 * not execute properly or code is limmited/not correct)
 */
void Encryption(int letters[alph], int orig_letters[alph])
{
   int i, I, size = 500;
...
/*
 * The following function takes two arrays from a previous function (ie an array containing original letters and another of their corresponding encrypted letters) and 
 * decrypts the message from encrypted.txt according to the two arrays, printing it to console and to origional.txt. The return is the exiting/ending of the 
 * function/program.
 * NB an incorrect decryption will be made if arrays have letters stored in incorrect order (ie functions that output arrays letters and orig_letters to this function does
 * not execute properly or code is limmited/not correct)
 */
void Decryption(int letters[alph], int orig_letters[alph])
{
   ...
   while(!feof(input))
   {
       ...
       /*
        ...
        printf("\nNew: %s\n",origional_text); //array origional_text (ie original message) is printed to console
